-- ============================================================
-- AkoraHub - Patch Phase 176 : rotation du WEBHOOK_SECRET (2e fois)
-- À exécuter une seule fois : Supabase Dashboard -> SQL Editor -> New query
--
-- Contexte (14/08) : même situation qu'en Phase 78 — l'ancien
-- WEBHOOK_SECRET n'est plus disponible côté utilisatrice (les secrets
-- Supabase ne sont jamais réaffichables une fois créés). On le remplace
-- partout où il est utilisé : les 9 fonctions déjà remises à jour en
-- Phase 78, PLUS `notify_push_on_variant_back_in_stock` (Phase 175,
-- créée après cette rotation précédente, donc pas encore couverte).
--
-- Nouvelle valeur : remplace TOUS les `<WEBHOOK_SECRET>` ci-dessous par
-- la même nouvelle valeur secrète (générée par toi, ex. un mot de passe
-- aléatoire long) avant d'exécuter ce script.
--
-- ⚠️ AVANT d'exécuter ce script : mets d'abord à jour le secret côté
-- Edge Function (Dashboard -> Edge Functions -> send-push-notification ->
-- Manage secrets -> WEBHOOK_SECRET -> colle la même nouvelle valeur).
-- Si l'ordre est inversé, les notifications échoueront silencieusement
-- (401) pendant quelques minutes entre les deux étapes — rien de cassé
-- de façon permanente.
--
...
-- Nouvelle depuis Phase 78 (Phase 175, conditionnements) — pas encore
-- couverte par la rotation précédente.
